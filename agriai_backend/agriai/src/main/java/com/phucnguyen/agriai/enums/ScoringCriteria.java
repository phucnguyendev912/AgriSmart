    ...
    DRUG_HAS_INGREDIENT(3, "Co du lieu hoat chat",
            plan -> plan.getDrug() != null
                    ...
                    && plan.getDrug().getIngredients().stream()
                            .anyMatch(drugIngredient -> drugIngredient.getIngredient() != null
                                    && !Boolean.TRUE.equals(drugIngredient.getIsDelete())
                                    && !Boolean.TRUE.equals(drugIngredient.getIngredient().getIsDelete()))),

    DISPLAY_DOSAGE(2, "Co huong dan lieu luong",
            plan -> plan.getDisplayDosage() != null && !plan.getDisplayDosage().isBlank()),
