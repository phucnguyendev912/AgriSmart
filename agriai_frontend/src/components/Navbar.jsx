                ...
                <button onClick={handleLogout} className="hidden md:flex items-center gap-3 pl-4 border-l border-slate-200 hover:bg-slate-50 transition-colors py-2 px-3 rounded-lg group" title="Đăng xuất">
                  <div className="flex flex-col items-end">
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-bold text-slate-900 leading-none group-hover:text-error transition-colors">{user.fullName || 'Người dùng'}</span>
                      <span className="material-symbols-outlined text-slate-400 text-sm group-hover:text-error">logout</span>
                    </div>
              ...
              {user ? (
                <>
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <p className="font-bold text-slate-900">{user.fullName || 'Nguoi dung'}</p>
                      <p className="text-xs text-slate-500">{user.roleName === 'ADMIN' ? 'Quan tri vien' : 'Nong dan tieu bieu'}</p>
                    </div>
                  </div>
                  <button onClick={handleLogout} className="p-2 text-error hover:bg-error-container rounded-lg">
