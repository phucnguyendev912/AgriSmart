const DrugTable = ({ 
  drugs, 
  page, 
  totalPages, 
  totalElements, 
  setPage, 
  onViewDetail,
  onEdit, 
  onDelete 
}) => {
  const formatDate = (dateString) => {
    if (!dateString) return '-';
    try {
      const date = new Date(dateString);
      return date.toLocaleDateString('vi-VN', {
        year: 'numeric',
        month: '2-digit',
        day: '2-digit',
        hour: '2-digit',
        minute: '2-digit'
      });
    } catch {
      return dateString;
    }
  };

  const formatIngredients = (ingredients) => {
    if (!ingredients || ingredients.length === 0) return '-';
    return ingredients
      .map(i => {
        const val = i.concentrationValue !== null && i.concentrationValue !== undefined ? i.concentrationValue : '';
        const unit = i.concentrationUnit || '';
        return `${i.ingredientName} ${val}${unit}`.trim();
      })
      .join(', ');
  };

  return (
    <>
      <div className="bento-card rounded-2xl overflow-hidden shadow-sm border border-outline-variant/10">
        {/* Desktop View */}
        <div className="hidden lg:block overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-surface-dim/30 border-b border-outline-variant/20">
                <th className="p-4 text-xs font-bold text-stone-500 uppercase tracking-widest">ID</th>
                <th className="p-4 text-xs font-bold text-stone-500 uppercase tracking-widest">Tên thuốc</th>
                <th className="p-4 text-xs font-bold text-stone-500 uppercase tracking-widest">Dạng thuốc</th>
                <th className="p-4 text-xs font-bold text-stone-500 uppercase tracking-widest">Nhà sản xuất</th>
                <th className="p-4 text-xs font-bold text-stone-500 uppercase tracking-widest">Hoạt chất</th>
                <th className="p-4 text-xs font-bold text-stone-500 uppercase tracking-widest">Trạng thái</th>
                <th className="p-4 text-xs font-bold text-stone-500 uppercase tracking-widest">Ngày tạo</th>
                <th className="p-4 text-xs font-bold text-stone-500 uppercase tracking-widest text-right">Thao tác</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-outline-variant/10">
              {drugs.length === 0 ? (
                <tr>
                  <td colSpan="8" className="p-8 text-center text-stone-500">
                    Không tìm thấy thuốc nào
                  </td>
                </tr>
              ) : (
                drugs.map((drug) => (
                  <tr key={drug.id} className="hover:bg-surface-dim/10 transition-colors group">
                    <td className="p-4">
                      <span className="font-mono text-xs font-medium text-stone-400">#{drug.id}</span>
                    </td>
                    <td className="p-4">
                      <span className="font-semibold text-on-surface">{drug.drugName}</span>
                    </td>
                    <td className="p-4">
                      <span className="text-stone-600 text-sm font-medium">{drug.formulation || '-'}</span>
                    </td>
                    <td className="p-4">
                      <span className="text-stone-600 text-sm font-medium">{drug.manufacturer || '-'}</span>
                    </td>
                    <td className="p-4 max-w-xs truncate">
                      <span className="text-stone-500 text-sm" title={formatIngredients(drug.ingredients)}>
                        {formatIngredients(drug.ingredients)}
                      </span>
                    </td>
                    <td className="p-4">
                      <span className={`px-2 py-1 rounded text-xs font-bold ${
                        drug.isActive 
                          ? 'bg-emerald-50 text-emerald-800' 
                          : 'bg-error/10 text-error'
                      }`}>
                        {drug.isActive ? 'Hoạt động' : 'Ngừng hoạt động'}
                      </span>
                    </td>
                    <td className="p-4">
                      <span className="text-stone-500 text-xs">{formatDate(drug.createdAt)}</span>
                    </td>
                    <td className="p-4 text-right">
                      <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button
                          onClick={() => onViewDetail(drug)}
                          className="w-8 h-8 rounded-full bg-surface hover:bg-emerald-50 flex items-center justify-center text-stone-500 hover:text-emerald-700 transition-colors"
                          title="Xem chi tiết"
                        >
                          <span className="material-symbols-outlined text-[18px]">visibility</span>
                        </button>
                        <button
                          onClick={() => onEdit(drug)}
                          className="w-8 h-8 rounded-full bg-surface hover:bg-primary/20 flex items-center justify-center text-stone-500 hover:text-primary transition-colors"
                          title="Sửa"
                        >
                          <span className="material-symbols-outlined text-[18px]">edit</span>
                        </button>
                        <button
                          onClick={() => onDelete(drug)}
                          className="w-8 h-8 rounded-full bg-surface hover:bg-error/20 flex items-center justify-center text-stone-500 hover:text-error transition-colors"
                          title="Xóa"
                        >
                          <span className="material-symbols-outlined text-[18px]">delete</span>
                        </button>
                      </div>
                    </td>
                  </tr>
                ))
              )}
            </tbody>
          </table>
        </div>

        {/* Mobile View */}
        <div className="lg:hidden divide-y divide-outline-variant/10">
          {drugs.length === 0 ? (
            <div className="p-8 text-center text-stone-500">
              Không tìm thấy thuốc nào
            </div>
          ) : (
            drugs.map((drug) => (
              <div key={drug.id} className="p-4">
                <div className="flex justify-between items-start mb-2">
                  <div>
                    <h3 className="font-bold text-on-surface">
                      {drug.drugName}
                    </h3>
                    <p className="text-xs text-stone-500 mt-1">
                      Nhà sản xuất: <span className="font-semibold text-stone-700">{drug.manufacturer || '-'}</span> 
                    </p>
                  </div>
                  <span className="font-mono text-xs text-stone-400">#{drug.id}</span>
                </div>
                
                <div className="mt-3 grid grid-cols-2 gap-2 text-xs text-stone-500">
                  <div>
                    <p className="text-[10px] font-bold text-stone-400 uppercase">Dạng thuốc</p>
                    <p className="font-medium text-stone-700 mt-0.5">{drug.formulation || '-'}</p>
                  </div>
                  <div>
                    <p className="text-[10px] font-bold text-stone-400 uppercase">Trạng thái</p>
                    <p className="font-medium mt-0.5">
                      <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${
                        drug.isActive 
                          ? 'bg-emerald-50 text-emerald-800' 
                          : 'bg-error/10 text-error'
                      }`}>
                        {drug.isActive ? 'Hoạt động' : 'Ngừng'}
                      </span>
                    </p>
                  </div>
                  <div className="col-span-2 mt-1">
                    <p className="text-[10px] font-bold text-stone-400 uppercase">Hoạt chất</p>
                    <p className="font-medium text-stone-600 mt-0.5 truncate">{formatIngredients(drug.ingredients)}</p>
                  </div>
                </div>

                <div className="flex justify-between items-center mt-4 pt-3 border-t border-outline-variant/10">
                  <div className="flex items-center gap-2">
                    <span className="text-[10px] text-stone-400">{formatDate(drug.createdAt)}</span>
                  </div>
                  <div className="flex gap-2">
                    <button 
                      onClick={() => onViewDetail(drug)} 
                      className="px-2.5 py-1.5 text-xs font-semibold rounded-lg bg-emerald-50 text-emerald-700 hover:bg-emerald-100 transition-colors"
                    >
                      Chi tiết
                    </button>
                    <button 
                      onClick={() => onEdit(drug)} 
                      className="px-2.5 py-1.5 text-xs font-semibold rounded-lg bg-primary/10 text-primary hover:bg-primary/20 transition-colors"
                    >
                      Sửa
                    </button>
                    <button 
                      onClick={() => onDelete(drug)} 
                      className="px-2.5 py-1.5 text-xs font-semibold rounded-lg bg-error/10 text-error hover:bg-error/20 transition-colors"
                    >
                      Xóa
                    </button>
                  </div>
                </div>
              </div>
            ))
          )}
        </div>

        {/* Pagination */}
        {totalPages > 1 && (
          <div className="p-4 border-t border-outline-variant/20 flex flex-col sm:flex-row items-center justify-between gap-4 bg-surface-dim/10">
            <span className="text-sm text-stone-500 font-medium">
              Hiển thị trang <span className="text-on-surface font-bold">{page + 1}</span> / {totalPages} (Tổng {totalElements})
            </span>
            <div className="flex gap-2">
              <button
                onClick={() => setPage(Math.max(0, page - 1))}
                disabled={page === 0}
                className="w-10 h-10 rounded-xl flex items-center justify-center bg-surface border border-outline-variant/30 text-stone-600 hover:bg-surface-dim hover:text-on-surface disabled:opacity-50 disabled:cursor-not-allowed transition-all"
              >
                <span className="material-symbols-outlined text-[20px]">chevron_left</span>
              </button>
              <button
                onClick={() => setPage(Math.min(totalPages - 1, page + 1))}
                disabled={page >= totalPages - 1}
                className="w-10 h-10 rounded-xl flex items-center justify-center bg-surface border border-outline-variant/30 text-stone-600 hover:bg-surface-dim hover:text-on-surface disabled:opacity-50 disabled:cursor-not-allowed transition-all"
              >
                <span className="material-symbols-outlined text-[20px]">chevron_right</span>
              </button>
            </div>
          </div>
        )}
      </div>
    </>
  );
};

export default DrugTable;
