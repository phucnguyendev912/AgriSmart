const IngredientTable = ({ ingredients, page, totalPages, totalElements, setPage, onEdit, onDelete }) => {
    ...
    <>
      <div className="bento-card rounded-2xl overflow-hidden shadow-sm border border-outline-variant/10">
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-surface-dim/30 border-b border-outline-variant/20">
                ...
                <th className="p-4 text-xs font-bold text-stone-500 uppercase tracking-widest">Tên hoạt chất</th>
              ...
              {ingredients.length === 0 ? (
                ...
                ingredients.map((item) => (
                  <tr key={item.id} className="hover:bg-surface-dim/10 transition-colors group">
                    ...
                    <td className="p-4">
                      <span className="font-semibold text-on-surface">{item.ingredientName}</span>
                    </td>
                    <td className="p-4 max-w-xs">
                      <span className="text-stone-500 text-sm truncate block" title={item.description}>
                        {item.description || '-'}
                      </span>
                    </td>
                    <td className="p-4">
        ...
        <div className="lg:hidden divide-y divide-outline-variant/10">
          {ingredients.length === 0 ? (
            <div className="p-8 text-center text-stone-500">Không tìm thấy hoạt chất nào</div>
          ) : (
            ingredients.map((item) => (
              <div key={item.id} className="p-4">
                <div className="flex justify-between items-start mb-2">
                  <div>
                    <h3 className="font-bold text-on-surface">{item.ingredientName}</h3>
          ...
          <div className="p-4 border-t border-outline-variant/20 flex flex-col sm:flex-row items-center justify-between gap-4 bg-surface-dim/10">
            ...
            <div className="flex gap-2">
              <button
                ...
                disabled={page === 0}
                className="w-10 h-10 rounded-xl flex items-center justify-center bg-surface border border-outline-variant/30 text-stone-600 hover:bg-surface-dim disabled:opacity-50 disabled:cursor-not-allowed transition-all"
              >
