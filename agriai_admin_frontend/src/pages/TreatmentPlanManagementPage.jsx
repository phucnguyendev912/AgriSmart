import { useState, useEffect } from 'react';
import { useTreatmentPlans } from '../features/treatment-plans/useTreatmentPlans';
import TreatmentPlanStats from '../features/treatment-plans/TreatmentPlanStats';
import TreatmentPlanTable from '../features/treatment-plans/TreatmentPlanTable';
import TreatmentPlanDetailModal from '../features/treatment-plans/TreatmentPlanDetailModal';
import TreatmentPlanFormModal from '../features/treatment-plans/TreatmentPlanFormModal';

const TreatmentPlanManagementPage = () => {
  const {
    treatmentPlans,
    stats,
    loading,
    error,
    page,
    size,
    totalPages,
    totalElements,
    simpleDiseases,
    simpleDrugs,
    simpleCropTypes,
    setPage,
    setSize,
    setFilters,
    createTreatmentPlan,
    updateTreatmentPlan,
    deleteTreatmentPlan
  } = useTreatmentPlans();

  const [successMessage, setSuccessMessage] = useState('');
  const [searchTerm, setSearchTerm] = useState('');
  const [selectedCropType, setSelectedCropType] = useState('');

  // Modals state
  const [isDetailOpen, setIsDetailOpen] = useState(false);
  const [isFormOpen, setIsFormOpen] = useState(false);
  const [selectedPlan, setSelectedPlan] = useState(null);

  // Handle auto search with input debounce
  useEffect(() => {
    const handler = setTimeout(() => {
      setFilters(prev => ({
        ...prev,
        treatmentName: searchTerm,
        cropTypeId: selectedCropType
      }));
      setPage(0); // Reset page to 0 on new search/filter
    }, 400);

    return () => clearTimeout(handler);
  }, [searchTerm, selectedCropType, setFilters, setPage]);

  // Clear success message after 3 seconds
  useEffect(() => {
    if (successMessage) {
      const timer = setTimeout(() => setSuccessMessage(''), 3000);
      return () => clearTimeout(timer);
    }
  }, [successMessage]);

  const handleAddClick = () => {
    setSelectedPlan(null);
    setIsFormOpen(true);
  };

  const handleViewDetailClick = (plan) => {
    setSelectedPlan(plan);
    setIsDetailOpen(true);
  };

  const handleEditClick = (plan) => {
    setSelectedPlan(plan);
    setIsFormOpen(true);
  };

  const handleDeleteClick = async (plan) => {
    if (window.confirm(`Bạn có chắc chắn muốn xóa phác đồ điều trị "${plan.treatmentName}" không?`)) {
      try {
        await deleteTreatmentPlan(plan.id);
        setSuccessMessage('Xóa phác đồ điều trị thành công');
      } catch (err) {
        console.error(err);
      }
    }
  };

  const handleFormSubmit = async (data) => {
    try {
      if (selectedPlan) {
        await updateTreatmentPlan(selectedPlan.id, data);
        setSuccessMessage('Cập nhật phác đồ điều trị thành công');
      } else {
        await createTreatmentPlan(data);
        setSuccessMessage('Thêm mới phác đồ điều trị thành công');
      }
      setIsFormOpen(false);
    } catch (err) {
      console.error(err);
      throw err;
    }
  };

  // Convert simple lists to match naming of Dropdowns in form
  const formattedDiseases = simpleDiseases.map(d => ({ id: d.id, name: d.name }));
  const formattedDrugs = simpleDrugs.map(d => ({ id: d.id, name: d.name }));

  return (
    <div className="flex-1 overflow-x-hidden overflow-y-auto bg-surface p-4 sm:p-6 lg:p-8">
      {/* Navigation & Header */}
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        <div>
          <nav className="flex items-center gap-2 text-xs text-stone-500 mb-2">
            <span>Hệ thống</span>
            <span className="material-symbols-outlined text-[10px]">chevron_right</span>
            <span className="text-emerald-700 font-medium">Quản lý phác đồ</span>
          </nav>
          <h1 className="text-3xl font-black text-on-surface tracking-tight">Quản lý Phác đồ điều trị</h1>
          <p className="text-stone-500 mt-2 font-medium">
            Thiết lập, chỉnh sửa và cấu hình chi tiết phác đồ chăm sóc & điều trị bệnh trên cây trồng.
          </p>
        </div>
        <button
          onClick={handleAddClick}
          className="flex items-center gap-2 px-6 py-3 rounded-xl font-bold text-white bg-emerald-600 hover:bg-emerald-700 shadow-md shadow-emerald-600/20 transition-all active:scale-95 w-full sm:w-auto justify-center"
        >
          <span className="material-symbols-outlined text-[20px]">add</span>
          Thêm Phác Đồ Mới
        </button>
      </div>

      {/* Notifications */}
      {error && (
        <div className="mb-6 bg-error/10 border border-error/20 text-error px-4 py-3 rounded-xl flex items-center gap-3">
          <span className="material-symbols-outlined">error</span>
          <p className="font-medium text-sm">{error}</p>
        </div>
      )}
      {successMessage && (
        <div className="mb-6 bg-emerald-50 border border-emerald-200 text-emerald-800 px-4 py-3 rounded-xl flex items-center gap-3 shadow-sm animate-in fade-in slide-in-from-top-2 duration-200">
          <span className="material-symbols-outlined text-emerald-700">check_circle</span>
          <p className="font-medium text-sm">{successMessage}</p>
        </div>
      )}

      {/* Stats Cards */}
      <div className="mb-8">
        <TreatmentPlanStats totalTreatmentPlans={stats.totalTreatmentPlans} />
      </div>

      {/* Main Table Area */}
      <div className="bg-surface rounded-2xl shadow-sm border border-outline-variant/20 p-4 sm:p-6">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-outline-variant/10 pb-6 mb-6">
          <h2 className="text-xl font-black text-on-surface">Danh sách phác đồ điều trị</h2>
        </div>

        {/* Filter controls */}
        <div className="flex flex-col md:flex-row gap-4 items-center justify-between mb-6">
          <div className="flex flex-col sm:flex-row gap-4 w-full md:max-w-xl">
            {/* Search Input */}
            <div className="relative w-full sm:w-72">
              <span className="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
                <span className="material-symbols-outlined text-stone-400 text-[20px]">search</span>
              </span>
              <input
                type="text"
                className="form-input pl-10 pr-4 py-2 text-sm rounded-lg border-outline-variant/30 bg-surface/50 w-full font-medium text-on-surface placeholder-stone-400 focus:border-emerald-500 focus:ring-emerald-500"
                placeholder="Tìm kiếm phác đồ..."
                value={searchTerm}
                onChange={(e) => setSearchTerm(e.target.value)}
              />
            </div>

            {/* Crop Type Filter Dropdown */}
            <div className="relative w-full sm:w-60">
              <select
                className="form-select text-sm rounded-lg border-outline-variant/30 bg-surface/50 w-full font-medium text-on-surface focus:border-emerald-500 focus:ring-emerald-500"
                value={selectedCropType}
                onChange={(e) => setSelectedCropType(e.target.value)}
              >
                <option value="">-- Lọc theo loại cây trồng --</option>
                {simpleCropTypes.map(c => (
                  <option key={c.id} value={c.id}>{c.name}</option>
                ))}
              </select>
            </div>
          </div>

          <div className="flex items-center gap-2 w-full md:w-auto justify-end">
            <span className="text-xs font-medium text-stone-500 uppercase tracking-wider">Hiển thị</span>
            <select
              className="form-select text-sm rounded-lg border-outline-variant/30 bg-surface/50 font-medium text-on-surface focus:border-emerald-500 focus:ring-emerald-500"
              value={size}
              onChange={(e) => setSize(Number(e.target.value))}
            >
              <option value={10}>10</option>
              <option value={20}>20</option>
              <option value={50}>50</option>
            </select>
          </div>
        </div>

        {/* List Data Table */}
        {loading && treatmentPlans.length === 0 ? (
          <div className="flex justify-center items-center py-20">
            <div className="w-8 h-8 border-4 border-emerald-600/30 border-t-emerald-600 rounded-full animate-spin"></div>
          </div>
        ) : (
          <TreatmentPlanTable
            treatmentPlans={treatmentPlans}
            page={page}
            totalPages={totalPages}
            totalElements={totalElements}
            setPage={setPage}
            onViewDetail={handleViewDetailClick}
            onEdit={handleEditClick}
            onDelete={handleDeleteClick}
          />
        )}
      </div>

      {/* Modals */}
      <TreatmentPlanDetailModal
        isOpen={isDetailOpen}
        onClose={() => setIsDetailOpen(false)}
        plan={selectedPlan}
      />

      {isFormOpen && (
        <TreatmentPlanFormModal
          key={selectedPlan ? `edit-${selectedPlan.id}` : 'create'}
          isOpen={isFormOpen}
          onClose={() => setIsFormOpen(false)}
          onSubmit={handleFormSubmit}
          plan={selectedPlan}
          diseases={formattedDiseases}
          drugs={formattedDrugs}
        />
      )}
    </div>
  );
};

export default TreatmentPlanManagementPage;
